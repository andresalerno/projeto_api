...
def grafico(dado):
    try:
        ...
        if dado == 'Temperatura':
            # VELHO data[dado] = data[dado].str.replace(',', '.').astype(float)
            data[dado] = data[dado].astype(float)
        elif dado == 'Volume Água (L)':
            # VELHO data[dado] = pd.to_numeric(data[dado].str.replace(',', '.'), errors='coerce')
            data[dado] = pd.to_numeric(data[dado], errors='coerce')
...
def obter_ultimos_valores(dado):
    try:
        ...
        df = pd.read_csv('app\\Files\\Dados.csv', encoding='ISO-8859-1', sep=';')
